...
export async function verifyAccessToken(token) {
    ...
    if (!decoded || typeof decoded === 'string') {
        throw new TokenVerificationError('Token is malformed or cannot be decoded', 'TOKEN_MALFORMED');
    }
    const { kid } = decoded.header;
    if (!kid) {
        throw new TokenVerificationError('Token header missing "kid" (key ID)', 'TOKEN_MALFORMED');
    }
    ...
    try {
        ...
        if (error instanceof Error && error.message.includes('not found')) {
            throw new TokenVerificationError(`Key "${kid}" not found in JWKS`, 'KEY_NOT_FOUND');
        }
    ...
    try {
        ...
        if (error instanceof jwt.JsonWebTokenError) {
            ...
            if (message.includes('issuer')) {
                throw new TokenVerificationError(`Invalid issuer. Expected: ${AUTH_CONFIG.ISSUER}`, 'TOKEN_INVALID_ISSUER');
            }
            if (message.includes('audience')) {
                throw new TokenVerificationError(`Invalid audience. Expected: ${AUTH_CONFIG.WIDGET_API_AUDIENCE}`, 'TOKEN_INVALID_AUDIENCE');
            }
            if (message.includes('signature')) {
                throw new TokenVerificationError('Invalid token signature', 'TOKEN_INVALID_SIGNATURE');
            }
        }
        throw new TokenVerificationError(`Token verification failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'TOKEN_INVALID_SIGNATURE');
    }
}
